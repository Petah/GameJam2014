...
class PlatformerControllerMovement {
	...
	var walkSpeed = 3.0;
	// when pressing "B1" button (control) we start running
	var runSpeed = 10.0;
...
function UpdateSmoothedMovementDirection () {	
	...
	if (controller.isGrounded) {
		...
		// Pick speed modifier
		if (Input.GetButton ("B1") && canControl)
...
function ApplyGravity () {
	// Apply gravity
	var jumpButton = Input.GetButton ("A1");
	...
	if ( ( jump.jumping  && Input.GetButtonUp("A1") && !jump.doubleJumping )  || ( !controller.isGrounded  && !jump.jumping && !jump.doubleJumping && movement.verticalSpeed  < -12.0  )  ) {
					jump.canDoubleJump = true;
	} 
 
	// if we can do a double jump, and we press the jump button, we do a double jump
	if ( jump.canDoubleJump && Input.GetButtonDown("A1") && !IsTouchingCeiling () ){
...
function Update () {
	if (Input.GetButtonDown ("A1") && canControl) {
